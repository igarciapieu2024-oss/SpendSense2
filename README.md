...
- Selection sort: for sorting the names in alphabetical order for then use the binary search which require sorted list. Running time was 0(n^2). We used this approach for the internal flow as we do not require that a special fast implementation and we want to avoid risking space management (in-place method) as we do in quicksort (at same time of quicksort is done recursion which occupies more memory). 
